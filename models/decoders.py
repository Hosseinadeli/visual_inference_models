...
class DecoderBurgess(nn.Module):
    def __init__(self, args, img_size, latent_dim=128):
        r"""Decoder of the model proposed in [1].
        ...
        self.img_size = img_size
        # Shape required to start transpose convs
        self.reshape = (hid_channels, 28, 28)
        n_chan = self.img_size[0]
        self.img_size = img_size

        
        # Fully connected layers
        self.lin1 = nn.Linear(latent_dim, 2*hidden_dim)
        self.lin2 = nn.Linear(2*hidden_dim, 4*hidden_dim)
        self.lin3 = nn.Linear(4*hidden_dim, 8*hidden_dim)
        self.lin4 = nn.Linear(8*hidden_dim, np.product(self.reshape))
...
#             self.convT_64 = nn.ConvTranspose2d(hid_channels, hid_channels, kernel_size, **cnn_kwargs)
        ...
        #self.convT2 = nn.ConvTranspose2d(2*hid_channels, 2*hid_channels, kernel_size, **cnn_kwargs)
        # self.convT3 = nn.ConvTranspose2d(2*hid_channels, 2*hid_channels, kernel_size, **cnn_kwargs)
        self.convT4 = nn.ConvTranspose2d(2*hid_channels, n_chan, kernel_size, **cnn_kwargs)
